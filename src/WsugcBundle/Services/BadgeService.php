...
class BadgeService
{
    ...
    {
        // $this->entityRepository = app('registry')->getManager('default')->getRepository(Badge::class);
        $this->entityRepository = getRepositoryLangue(Badge::class);
    }
    ...
    public function getBadgeList($filter,$cols,  $page = 1, $pageSize = -1, $orderBy=[])
    {
        if(!$orderBy){
            //按排序，小的在前。
            $orderBy=[
        ...
        app('log')->debug('lists-原始******'.var_export($lists['list'],true));
        ...
        foreach ($lists['list'] as &$v) {
            $v=$this->formatDetail($v,false,$wechatUserService);
            ksort($v);
        }
        if($v??null){
            unset($v);
        ...
        app('log')->debug('lists******-formatDetail后'.var_export($lists['list'],true));

        if($filter['badge_name|contains']??null){
            //有名称搜索，一模一样的排第一位
            $allRank=[];
            foreach($lists['list'] as $k=>$v){
                app('log')->debug('lists******'.var_export($lists['list'],true));
                if($v['badge_name']==$filter['badge_name|contains']){
                    app('log')->debug('lists******相等于'.var_export($filter['badge_name|contains'],true));
     ...
     */
    public function getBadgeDetail($filter,$user_id=""){
        $badgeInfo=$this->getInfo($filter);
        if($badgeInfo && ($badgeInfo['badge_id']??null)){
            $wechatUserService = new WechatUserService();
            $badgeInfo=$this->formatDetail($badgeInfo,true,$wechatUserService);
        }
        ksort($badgeInfo);
        return $badgeInfo;
    }
