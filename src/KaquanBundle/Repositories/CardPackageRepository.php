...
class CardPackageRepository extends EntityRepository
{
    use MagicLangTrait;
    public $table = "card_package";
    public $cols = ['package_id','company_id','title','package_describe','limit_count','get_num','row_status','created','updated'];
    public $prk = 'package_id';
    ...
    public function insertGetId($data)
    {
        ...
        $this->getLangService()->addMultiLangByParams($package_id,$data,$this->table);
        return $package_id;
    }

    public function incrGetNum(int $companyId, int $packageId)
    {
        $conn = app('registry')->getConnection('default');
        $updateSql = 'UPDATE ' . $this->table . ' SET get_num = get_num + 1 WHERE company_id =' . $companyId . ' and package_id=' . $packageId;
