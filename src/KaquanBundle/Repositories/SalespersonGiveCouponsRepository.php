...
class SalespersonGiveCouponsRepository extends EntityRepository
{
    public $table = "kaquan_salesperson_give_coupons";
    public $cols = ['id','company_id','salesperson_id','salesperson_name','user_id','user_name','coupons_id','coupons_name','number','status','fail_reason','give_time','updated'];
    /**
     * 新增
     *
     * @param array $data
    ...
    {
        $entity = new SalespersonGiveCoupons();
     ...
     * 用户id分组查询发给用户的优惠券数量
     */
    public function getCountByUserId($filter)
    {
        $conn = app('registry')->getConnection('default');
        $qb = $conn->createQueryBuilder();
        $qb->select('user_id, count(user_id) count')
            ->from($this->table)
            ->groupBy('user_id');
        if ($filter) {
            $this->_filter($filter, $qb);
    ...
    public function getCountGroupByCouponsId($filter)
    {
        $conn = app('registry')->getConnection('default');
        $qb = $conn->createQueryBuilder();
        $qb->select('coupons_id, count(coupons_id) count')
            ->from($this->table)
            ->groupBy('coupons_id');
        if ($filter) {
            $this->_filter($filter, $qb);
