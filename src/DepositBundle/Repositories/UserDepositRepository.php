...
class UserDepositRepository extends EntityRepository
{
    public $table = "user_deposit";
    public $cols = ['deposit_id','company_id','user_id','money','created','updated'];
    /**
     * 新增
     *
     * @param array $data
    ...
    public function getUserDepositListByCompanyList(array $filter, $cols = 'deposit_id,company_id,user_id,money')
    {
        $conn = app('registry')->getConnection('default');
        $qb = $conn->createQueryBuilder()->select($cols)->from($this->table);
        $qb = $this->_filter($filter, $qb);
    ...
    public function insertUserDeposit(array $data): bool
    {
        $entity = new UserDeposit();
        $em = $this->getEntityManager();
        ...
        foreach ($data as $datum) {
            $entity = $this->setColumnNamesData($entity, $datum);
            $em->persist($entity);
            $em->flush();
            $em->clear();
        }
