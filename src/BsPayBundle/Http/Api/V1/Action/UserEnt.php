 ...
 * 企业用户
 */
class UserEnt extends Controller
{
    /**
     * @SWG\Post(
     *     path="/bspay/user_ent/create",
     *     summary="创建企业用户对象",
     *     tags={"汇付斗拱"},
     *     description="创建企业用户对象",
     *     operationId="user_ent_create",
     ...
     *     @SWG\Parameter( name="branch_code", in="formData", description="支行联行号", required=false, type="string"),
    ...
    public function create(Request $request)
    {
        ...
        $params['legal_cert_no'] = trim($params['legal_cert_no'] ?? '') ?? '';
        $params['card_no'] = trim($params['card_no'] ?? '') ?? '';
        $rules = [
            'reg_name' => ['required', '企业名称必填'], //截取30个字符
            'license_code' => ['required', '统一社会信用代码必填'],
            'license_validity_type' => ['required', '营业期限类型必填'],
            'license_begin_date' => ['required', '成立日期必填'],
            'license_end_date' => ['required_if:license_validity_type,0', '营业期限必填'],
            'license_regions_id' => ['required', '住所省市区必填'],
            'reg_detail' => ['required', '住所详细地址地址必填'],
            'legal_name' => ['required', '法人姓名必填'],
            'legal_cert_no' => ['required|size:18', '法人身份证号码必须是18位'],
            'legal_cert_validity_type' => ['required', '法人身份证有效期类型必填'],
            'legal_cert_begin_date' => ['required', '法人身份证开始有效期必填'],
            'legal_cert_end_date' => ['required_if:legal_cert_validity_type,0', '法人身份证结束有效期必填'],
            // 'contact_name' => ['required|size:11', '联系人姓名必填'],
            'contact_mobile' => ['required|size:11', '手机必须是11位'],
            'ent_type' => ['required', '企业类型必填'],
            'card_type' => ['required|in:0,1', '结算银行账户类型错误'],
            // 'card_name' => ['required_if:card_type,1|max:20', '持卡人姓名必填'],// 对公，使用企业名称；对司法人，使用法人姓名
            'card_no' => ['required', '银行卡号必填'],
            'card_regions_id' => ['required', '开户行所在省市必填'],
            'branch_code' => ['required_if:card_type,0', '支行联行号必填'],
            'branch_name' => ['required_if:card_type,0', '支行名称必填'],
            // 'cert_no' => ['required_if:card_type,1', '持卡人身份证号必填'],// 使用法人身份证
        ...
        $userEntService = new UserEntService();
        $params = $userEntService->checkParams($params, true);//校验参数
        $result = $userEntService->createUser($params);
        if (!$result) {
    ...
    public function modify(Request $request)
    {
        ...
        $userEntService = new UserEntService();
        $params = $userEntService->checkParams($params, false);//校验参数
        $result = $userEntService->modifyUser($params);
        if (!$result) {
    ...
    public function update(Request $request)
    {
        ...
        $userEntService = new UserEntService();
        $params = $userEntService->checkParams($params, false);//校验参数
        $result = $userEntService->updateUser($params);
     ...
     *                   @SWG\Property(property="branch_code", type="string", description="支行联行号"),
    ...
    public function get(Request $request)
    {
        ...
        $result['ent_type'] = (string)$result['ent_type'];
