...
class PromotionsItemsTagRepository extends EntityRepository
{
    public $table = "promotions_items_tag";
    public $cols = ['id','promotion_id','item_id','goods_id','tag_type','item_type','start_time','end_time','company_id', 'is_all_items', 'activity_price'];
    ...
    {
        $isAllItems = false;
        if (isset($filter['is_all_items']) && $filter['is_all_items'] == 1) {
        ...
        if ($isAllItems) {
            $qb = $qb->orWhere($qb->expr()->andX(
                $qb->expr()->eq('is_all_items', 1)
            ));
        }
        return $qb;
    }
