...
class MarketingGiftItemsRepository extends EntityRepository
{
    ...
    private function setColumnNamesData($entity, $data)
    {
        ...
        if (isset($data["filter_full"]) && $data["filter_full"]) {
            if (!isset($data['condition_type']) || !$data['condition_type']) {
                throw new ResourceException(trans("PromotionsBundle.no_promotion_rule_specified"));
            }

            if ($data['condition_type'] == 'totalfee') {
                $entity->setFilterFull(bcmul($data["filter_full"], 100));
            } else {
    ...
    private function getColumnNamesData($entity)
    {
        return [
            ...
            'filter_full' => $entity->getConditionType() == 'totalfee' ? bcdiv($entity->getFilterFull(), 100, 2) : strval($entity->getFilterFull()),
