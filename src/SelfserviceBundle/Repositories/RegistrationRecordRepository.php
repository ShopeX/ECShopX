...
class RegistrationRecordRepository extends EntityRepository
{
    use MagicLangTrait;
    public $table = "selfservice_registration_record";
    public $cols = ['record_id','activity_id','user_id','mobile','wxapp_appid','open_id',
        'status','content','reason','created','updated','company_id',
        'verify_time','verify_operator','true_name','form_mobile','group_no',
        'distributor_id', 'record_no', 'form_id', 'get_points', 'verify_code', 'is_white_list','remark'];
    public $prk = 'record_id';
    ...
    public function getJoinActivityUserNum($companyId, $activityId)
    {
        if (!$activityId) {
            return 0;
        }
        ...
        array_walk($activityIds, function (&$colVal) use ($criteria) {
            $colVal = $criteria->expr()->literal($colVal);
        });
        $criteria->select('count(user_id) as num,activity_id')
            ->from($this->table)
            ->where($criteria->expr()->eq('company_id', $companyId))
            ->where($criteria->expr()->in('activity_id', $activityIds))
            ->groupBy('activity_id');
        ...
        return array_column($list, 'num', 'activity_id');
    }
}
