...
/**
 * 订单正向同步 {@see ShuyunOpenPlatformOrderGatewayActions::GATEWAY_ACTION_TRADE_SYNC}；body 为订单对象 JSON 数组；单次最多 50 条。
 */
class ShuyunOpenPlatformTradeSyncService
{
    ...
    /**
     * @param  list<array<string, mixed>>  $orders  已通过业务组装的 trade 订单对象（与整理表字段一致）
     */
    public function syncValidatedTradeOrders(int $companyId, string $platformHeader, array $orders): bool
    {
        ...
        if ($platformNorm === '') {
            Log::channel(self::LOG_CHANNEL)->warning('Shuyun trade.sync skipped: empty platform header.', [
        ...
        $chunks = array_chunk($orders, self::BATCH_SIZE);

        try {
            foreach ($chunks as $chunk) {
                $client->postJson(ShuyunOpenPlatformOrderGatewayActions::GATEWAY_ACTION_TRADE_SYNC, $chunk, $tokenStr, $platformNorm);
            }
    ...
    {
        Log::channel(self::LOG_CHANNEL)->error('Shuyun trade.sync failed.', [
            'company_id' => $companyId,
            'exception' => $kind,
            'code' => $code,
            'message' => $message,
