...
/**
 * {@see NormalOrderPaySuccessEvent} → 数云线下权益核销 USED（§4.3）。
 */
final class ShuyunOfflineBenefitResultPushOnPaySuccessListener
{
    public function handle(NormalOrderPaySuccessEvent $event): void
    {
        ...
        if (array_key_exists('user_id', $payload) && (int) $payload['user_id'] === 0) {
            Log::channel('shuyun_open_platform')->info('Shuyun offline_benefit_consume dispatch skipped: user_id is zero.', [
        ...
        if (array_key_exists('total_fee', $payload) && (int) $payload['total_fee'] <= 0) {
            Log::channel('shuyun_open_platform')->info('Shuyun offline_benefit_consume dispatch skipped: total_fee is zero (fen).', [
                'company_id' => $companyId,
            ]);
        ...
        app(ShuyunOfflineBenefitConsumePushService::class)->handlePaySuccess($companyId, $orderId);
    }
