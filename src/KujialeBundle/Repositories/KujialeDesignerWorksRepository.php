<?php
namespace KujialeBundle\Repositories;
...
class KujialeDesignerWorksRepository extends EntityRepository
{

    public $table = "kujiale_designer_works";
    public $cols = ['id','design_name','cover_pic','is_origin','is_excellent','is_real_excellent','is_top','design_id','plan_id','comm_name','city','name','tag_id','design_pano_url','user_avatar','email','user_name','user_id','organization_id','created','updated','view_count','like_count','ku_created'];
    /**
     * 新增
     *
     * @param array $data
    ...
    {
        $entity = new KujialeDesignerWorks();
     ...
     * @param $filter 更新的条件
     */
    public function getLists($filter, $page = 1, $pageSize = -1, $orderBy = array())
    {
        $conn = app('registry')->getConnection('default');
        $qb = $conn->createQueryBuilder()->select($this->cols)->from($this->table);
        $qb = $this->_filter($filter, $qb);
        if ($orderBy) {
     ...
     * @param $filter
     */
    public function countView($filter){
        $entity = $this->findOneBy($filter);
        if( !$entity ) {
            throw new ResourceException("未查询到更新数据");
        }

        $count = $this->getColumnNamesData($entity)['view_count'];
