...
class TaskBrokerageRepository extends EntityRepository
{
    public $table = "popularize_task_brokerage";
    public $cols = ['id','item_id','user_id', 'item_name', 'num', 'item_spec_desc', 'order_id', 'price','buy_user_id','company_id','status','plan_date','created','updated'];
    /**
     * 新增
     *
     * @param array $data
    ...
    public function getMonthCount(array $filter, $page = 1, $pageSize = -1, $orderBy = array(), $groupBy = array())
    {
        $conn = app("registry")->getConnection("default");
        $qb = $conn->createQueryBuilder()->select('*, sum(price) as total_fee, sum(num) as _count')->from($this->table);

        $qb = $this->_filter($filter, $qb);

        if (!$groupBy) {
            $groupBy = ['item_id', 'company_id', 'user_id', 'status'];
        }
