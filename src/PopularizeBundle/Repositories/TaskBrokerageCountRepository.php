...
class TaskBrokerageCountRepository extends EntityRepository
{
    public $table = "popularize_task_brokerage_count";
    public $cols = ['id','rebate_type','item_id','item_bn','user_id','company_id', 'total_fee', 'item_name', 'item_spec_desc', 'rebate_conf','finish_num','wait_num','close_num','plan_date','created','updated', 'rebate_money'];
    /**
     * 新增
     *
     * @param array $data
    ...
    public function getRebateMoneyTotal($filter)
    {
        ...
        $qb->select('SUM(rebate_money) as rebate_money')->from($this->table);
