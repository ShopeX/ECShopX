...
class SalespersonPopularizeRecordStatisticsJob extends Job
{
    public $data;
    ...
    public function handle()
    {
        ...
        app('log')->debug('导购数据推广统计开始');
        foreach ($salespersons as $v) {
            app('log')->debug('导购数据推广统计开始=>:'.var_export($v, 1));
            try {
                $statisticService->recordSalespersonPopularizeStatistics($v['company_id'], $v['salesperson_id'], $yesterdayDate);
            } catch (\Exception $e) {
                app('log')->debug('导购数据推广统计error:'  . $e);
            }
        }
        app('log')->debug('导购数据推广统计结束');
        return true;
    }
}
