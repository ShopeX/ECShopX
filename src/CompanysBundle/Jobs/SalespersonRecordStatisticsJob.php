...
class SalespersonRecordStatisticsJob extends Job
{
    public $data;
    ...
    public function handle()
    {
        ...
        foreach ($salespersons as $v) {
            app('log')->debug('导购数据统计开始=>:'.var_export($v, 1));
            try {
//                $statisticService->recordSalespersonStatistics($v['company_id'], $v['salesperson_id'], 'service', $yesterdayDate);
                $statisticService->recordSalespersonStatistics($v['company_id'], $v['salesperson_id'], 'normal', $yesterdayDate);
            } catch (\Exception $e) {
                app('log')->debug('导购数据统计error:'  . $e);
            }
            app('log')->debug('导购数据统计结束');
        }
        return true;
    }
}
