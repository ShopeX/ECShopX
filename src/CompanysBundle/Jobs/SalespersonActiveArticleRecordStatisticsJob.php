...
class SalespersonActiveArticleRecordStatisticsJob extends Job
{
    public $data;
    ...
    public function handle()
    {
        ...
        foreach ($salespersons as $v) {
            app('log')->debug('活动转发数统计开始=>:'.var_export($v, 1));
            try {
                $statisticService->recordActiveArticleStatistics($v['company_id'], $v['salesperson_id'], $yesterdayDate);
            } catch (\Exception $e) {
                app('log')->debug('活动转发数统计error:'  . $e);
            }
            app('log')->debug('活动转发数统计结束');
        }
        return true;
    }
}
