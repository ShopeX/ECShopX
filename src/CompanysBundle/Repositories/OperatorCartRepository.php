...
class OperatorCartRepository extends EntityRepository
{
    public $table = "companys_operator_cart";
    public $cols = ['cart_id','company_id','distributor_id','operator_id','item_id','num','is_checked','special_type'];
    /**
     * 新增
     *
     * @param array $data
    ...
    public function getLists($filter, $cols = '*', $page = 1, $pageSize = -1, $orderBy = array())
    {
        ...
        foreach ($lists as &$value) {
            $value['shop_id'] = $value['distributor_id'];
            $value['shop_type'] = 'shop_offline';
        }
        return $lists;
    }
    ...
    public function batchInsert(array $data)
    {
        ...
        foreach ($data[0] as $columnName => $value) {
            if (in_array($columnName, $this->cols)) {
               $columns[] = $columnName;
            }
        }
