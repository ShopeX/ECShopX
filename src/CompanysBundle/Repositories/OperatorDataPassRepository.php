...
class OperatorDataPassRepository extends EntityRepository
{
    public $table = "operator_data_pass";
    public $cols = ['pass_id','company_id','operator_id','status','start_time','end_time','rule','reason','remarks','create_time','approve_time','is_closed','merchant_id'];
    /**
     * 新增
     *
     * @param array $data
    ...
    public function getListsJoinOp($filter, $cols = '*', $page = 1, $pageSize = -1, $orderBy = [])
    {
        $conn = app('registry')->getConnection('default');
        $qb = $conn->createQueryBuilder()->select($cols)->from($this->table, 'p');
        $qb = $this->_filter($filter, $qb);
        if ($orderBy) {
            foreach ($orderBy as $filed => $val) {
                $qb->addOrderBy($filed, $val);
        ...
        $qb->leftjoin('p', 'operators', 'o', 'p.operator_id = o.operator_id');
        $lists = $qb->execute()->fetchAll();
        return $lists;
    }
     ...
     * 统计数量
     */
    public function countJoinOp($filter)
    {
        $conn = app('registry')->getConnection('default');
        $qb = $conn->createQueryBuilder();
        $qb->select('count(pass_id)')
