...
{
    public $table = 'companys_roles';
    public $module = 'companys_roles'; // 多语言对应的模块
    public $primaryKey = 'role_id'; // 主键，对应data_id
    public $langField = [
        'role_name'
    ]; // 多语言字段
    ...
    private function setColumnNamesData($entity, $data)
    {
        ...
        if (isset($data["role_source"]) && $data["role_source"]) {
            $entity->setRoleSource($data["role_source"]);
        }
        if (isset($data["permission"]) && $data["permission"]) {
            $entity->setPermission(json_encode($data["permission"]));
        }
    ...
    private function getColumnNamesData($entity)
    {
        return [
            ...
            'role_name' => $entity->getRoleName(),
            'role_source' => $entity->getRoleSource(),
            'permission' => json_decode($entity->getPermission(), true),
