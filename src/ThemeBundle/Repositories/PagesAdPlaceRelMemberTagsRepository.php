...
/**
 * PagesAdPlaceRelMemberTagsRepository
 */
class PagesAdPlaceRelMemberTagsRepository extends EntityRepository
{
    public $table = 'pages_ad_place_rel_member_tags';
    public $cols = ['company_id', 'ad_place_id', 'tag_id'];
    ...
    {
        $entity = new PagesAdPlaceRelMemberTags();
    ...
    public function getRelTagsByAdPlaceIds($adPlaceIds)
    {
        $conn = app('registry')->getConnection('default');
        $qb = $conn->createQueryBuilder();
        $qb->from($this->table, 'r')
            ->leftJoin('r', 'members_tags', 't', 'r.tag_id = t.tag_id')
            ->where($qb->expr()->in('r.ad_place_id', $adPlaceIds));
        $tagList =$qb->select('r.ad_place_id, r.tag_id, t.tag_name')->execute()->fetchAll();
        $relTags = [];
        foreach ($tagList as $tag) {
            if (!isset($relTags[$tag['ad_place_id']])) {
                $relTags[$tag['ad_place_id']] = [['tag_id' => $tag['tag_id'], 'tag_name' => $tag['tag_name']]];
            } else {
                $relTags[$tag['ad_place_id']][] = ['tag_id' => $tag['tag_id'], 'tag_name' => $tag['tag_name']];
            }
        }
