...
class ThemePcTemplateContentRepository extends EntityRepository
{
    public $table = "theme_pc_template_content";
    public $cols = ['theme_pc_template_content_id','company_id','theme_pc_template_id','name','params','created','updated','sort_by'];
   
    public $module = 'theme_pc_template_content'; // 多语言对应的模块
    public $primaryKey = 'theme_pc_template_content_id'; // 主键，对应data_id
    public $langField = [
        'name','params'
    ]; // 多语言字段
    ...
    {
        $entity = new ThemePcTemplateContent();
        return $entity;
    }
     ...
     * @param $filter 更新的条件
     */
    public function lists($filter, $cols = '*', $page = 1, $pageSize = -1, $orderBy = array('sort_by' => 'ASC'))
    {
    ...
    public function count($filter)
    {
        ...
        $qb->select('count(theme_pc_template_content_id)')
             ->from($this->table);
        if ($filter) {
