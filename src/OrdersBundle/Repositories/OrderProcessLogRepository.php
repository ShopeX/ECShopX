...
class OrderProcessLogRepository extends EntityRepository
{
    public $table = "orders_process_log";
    public $cols = ['id','order_id','company_id','supplier_id','operator_type','operator_id','operator_name','remarks','detail','params','is_show','pics','delivery_remark','create_time','update_time'];
    /**
     * 新增
     *
     * @param array $data
    ...
    public function getLists($filter, $cols = '*', $page = 1, $pageSize = -1, $orderBy = array())
    {
        ...
        foreach ($lists as $k => $v) {
            $lists[$k]['operator_name'] = fixeddecrypt($v['operator_name']);
        }
        return $lists;
    }
    ...
    public function lists($filter, $cols = '*', $page = 1, $pageSize = -1, $orderBy = array())
    {
        ...
        foreach ($result['list'] as $k => $v) {
            $result['list'][$k]['operator_name'] = fixeddecrypt($v['operator_name']);
        }
        return $result;
    }
