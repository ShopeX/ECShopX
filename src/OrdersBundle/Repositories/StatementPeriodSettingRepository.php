...
class StatementPeriodSettingRepository extends EntityRepository
{
    public $table = 'statement_period_setting';
    public $cols = ['id', 'company_id', 'merchant_id', 'distributor_id', 'period', 'supplier_id', 'merchant_type'];
    /**
     * 新增
     *
     * @param array $data
    ...
    {
        $entity = new StatementPeriodSetting();
    ...
    public function getLists($filter, $cols = '*', $page = 1, $pageSize = -1, $orderBy = array())
    {
        ...
        array_walk($lists, function(&$row) {
            $row['period'] = json_decode($row['period']);
        });
