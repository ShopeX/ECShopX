...
class StatementsRepository extends EntityRepository
{
    public $table = 'statements';
    public $cols = [
        'id', 'company_id', 'merchant_id', 'distributor_id', 'statement_no', 
        'order_num', 'total_fee', 'freight_fee', 'intra_city_freight_fee', 
        'rebate_fee', 'refund_fee', 'statement_fee', 'start_time', 'end_time', 
        'supplier_id', 'merchant_type', 'point_fee', 'refund_num', 
        'refund_point', 'refund_cost_fee','confirm_time', 'statement_time', 
        'statement_status'
    ];
    /**
     * 新增
    ...
    private function _filter($filter, $qb)
    {
        foreach ($filter as $field => $value) {
            ...
            if (count($list) > 1) {
                ...
                if ($k == 'notnull') {
                    $qb->andWhere($qb->expr()->isNotNull($v));
                    continue;
                }
                if (is_array($value)) {
    ...
    public function getSum($field, $filter) {
        $conn = app('registry')->getConnection('default');
        $qb = $conn->createQueryBuilder();
        $qb->select('sum('.$field.')')
            ->from($this->table);
