...
class OrderProfitRepository extends EntityRepository
{
    public $table = "orders_rel_profit";
    public $cols = ['id', 'order_id', 'order_profit_status', 'company_id', 'total_fee', 'pay_fee', 'profit_type', 'user_id', 'dealer_id', 'distributor_id', 'order_distributor_id', 'distributor_nid', 'seller_id', 'popularize_distributor_id', 'popularize_seller_id', 'proprietary', 'popularize_proprietary', 'dealers', 'distributor', 'seller', 'popularize_distributor', 'popularize_seller', 'commission', 'rule', 'created', 'updated', 'plan_close_time'];
    /**
     * 新增
     *
     * @param array $data
    ...
    private function _filter($filter, $qb)
    {
        foreach ($filter as $field => $value) {
            if ('salesperson_id' == $field) {
                $qb = $qb->andWhere($qb->expr()->orX(
                    $qb->expr()->eq('seller_id', $qb->expr()->literal($value)),
                    $qb->expr()->eq('popularize_seller_id', $qb->expr()->literal($value))
                ));
            } else {
                $list = explode('|', $field);
                if (count($list) > 1) {
    ...
    {
        $conn = app('registry')->getConnection('default');
        $qb = $conn->createQueryBuilder()->select($cols)->from($this->table, 'op');
        $qb = $this->_filter($filter, $qb);
        if ($orderBy) {
     ...
     * @param $filter 更新的条件
     */
    public function lists($filter, $cols = 'op.*', $page = 1, $pageSize = -1, $orderBy = array())
    {
        ...
        if ($result['total_count'] > 0) {
            ...
            ->select($cols)
            ->from($this->table, 'op')
            ->leftJoin('op', 'orders_normal_orders', 'ono', 'op.order_id = ono.order_id');
            $qb = $this->_filter($filter, $qb);
            if ($orderBy) {
                foreach ($orderBy as $filed => $val) {
                    $qb->addOrderBy($filed, $val);
