...
use OpenapiBundle\Services\Order\DeliveryService;
...
class Delivery extends Controller
{
    ...
    {
        $params = $request->all('order_id', 'item_info', 'lc_code', 'l_code', 'order_sn');
        $params['item_info'] = json_decode($params['item_info'], true);
        if (!$params['item_info'] || !is_array($params['item_info'])) {
            throw new ErrorException(ErrorCode::SERVICE_PARAMS_FORMAT_ERROR, '发货商品信息格式错误');
        }
        foreach ($params['item_info'] as $item) {
            if (!is_array($item)) {
                throw new ErrorException(ErrorCode::SERVICE_PARAMS_FORMAT_ERROR, '发货商品信息格式错误');
            }
        }
        ...
        $rules = [
            ...
            'item_info.*.name' => ['required', '发货商品信息的商品名称必填'],
            'item_info.*.sku_id' => ['required', '发货商品信息的商品货号必填'],
            'item_info.*.qty' => ['required', '发货商品信息的发货数量必填'],
        ...
        if ($tradeInfo['delivery_status'] == 'DONE') {
            throw new ErrorException(ErrorCode::ORDER_HANDLE_EXIST, '订单已发货，请勿重复发货');
        }
        ...
        if ($result) {
            ...
            throw new ErrorException(ErrorCode::ORDER_HANDLE_ERROR);
        }
    }
    ...
    public function doOrderDelivery($order, $data)
    {
        try {
            ...
            if (empty($noDelivery) && !empty($emptyDelivery)) {
                ...
                throw new ErrorException(ErrorCode::ORDER_HANDLE_ERROR, '发货商品有误');
            }
    ...
    /**
     * @SWG\Get(
     *     path="/ecx.order.deliver.get",
     *     summary="订单发货单列表",
     *     tags={"订单"},
     *     description="查询单条订单的发货单列表",
     *     @SWG\Parameter( in="query", type="string", required=true, name="method", description="方法名称 ecx.order.deliver.get" ),
     *     @SWG\Parameter( in="query", type="string", required=true, name="app_key", description="app_key" ),
     *     @SWG\Parameter( in="query", type="string", required=true, name="version", description="版本号" ),
     *     @SWG\Parameter( in="query", type="string", required=true, name="timestamp", description="请求时间" ),
     *     @SWG\Parameter( in="query", type="string", required=true, name="sign", description="签名" ),
     ...
     *          @SWG\Property( property="message", type="string", example="操作成功", description="提示信息"),
     *          @SWG\Property( property="data", type="object",
     *                  @SWG\Property( property="delivery_num", type="string", example="0", description="已发货子单数量"),
     *                  @SWG\Property( property="list", type="array",
     *                      @SWG\Items( type="object",
     *                          @SWG\Property( property="delivery_id", type="string", example="1", description="发货单ID"),
     ...
     *                          @SWG\Property( property="delivery_code", type="string", example="SF1234567890", description="快递单号"),
     *                          @SWG\Property( property="delivery_time", type="string", example="2021-04-26 15:37:10", description="发货时间"),
     *                          @SWG\Property( property="items", type="array",
     *                              @SWG\Items( type="object",
     *                                  @SWG\Property( property="item_name", type="string", example="测试商品", description="商品名称"),
     ...
     *                  @SWG\Property( property="status_msg", type="string", example="已发货", description="发货状态描述"),
     *                  @SWG\Property( property="status", type="string", example="1", description="发货状态 0:未发货，1:已发货"),
     *                  
     *              ),
     *          ),
     *     )),
     ...
     * )
     */
    public function getOrderDeliveryList(Request $request)
    {
        ...
        $result = $deliveryService->getDeliveryItems($params);
        return $this->response->array($result);
    }
}
