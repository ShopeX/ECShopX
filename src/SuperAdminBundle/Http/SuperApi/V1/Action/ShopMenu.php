...
class ShopMenu extends Controller
{
     ...
     *     @SWG\Parameter(name="company_id",in="query",description="公司id",required=false,type="integer"),
    ...
    public function getShopMenu(Request $request)
    {
        ...
        $filter['disabled'] = 0;
        $filter['company_id'] = $request->input('company_id', 0);
    ...
    public function uploadMenu(Request $request)
    {
        ...
        if (!empty($company_id)) {
            foreach ($menus  as $k => $v) {
                $menus[$k]['company_id'] = $company_id;
            }
        }

        $shopMenuService = new ShopMenuService();
        $data = $shopMenuService->uploadMenus($menus, $company_id);
    ...
    public function downShopMenu(Request $request)
    {
        ...
        if ($filter['version'] == 1) {
            $filename = 'platform_menu.json';
        }
        if ($filter['version'] == 2) {
            $filename = 'it_menu.json';
        }
        if ($filter['version'] == 3) {
            $filename = 'shop_menu.json';
        }
        if ($filter['version'] == 5) {
            $filename = 'dealer_menu.json';
        }
        if ($filter['version'] == 6) {
            $filename = 'merchant_menu.json';
        }
        if ($filter['version'] == 7) {
            $filename = 'supplier_menu.json';
        }
    ...
    public function updateShopMenu(Request $request)
    {
        ...
        if (!$requestData['shopmenu_id']) {
            throw new DeleteResourceFailedException("shopmenu_id参数错误");
        }
        if (!isset($requestData['company_id']) || $requestData['company_id'] < 0) {
            throw new DeleteResourceFailedException("company_id必填或错误");
        }
     ...
     * )
     */
    public function deleteShopMenu($shopmenuId, Request $request)
    {
        if (!$shopmenuId) {
            throw new DeleteResourceFailedException("缺少菜单id");
        }
