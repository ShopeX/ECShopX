...
class HfpayCashRecordRepository extends EntityRepository
{
    public $table = "hfpay_cash_record";
    public $cols = ['hfpay_cash_record_id','company_id','distributor_id','order_id','user_id','operator_id','user_cust_id','trans_amt','cash_type','bind_card_id','real_trans_amt','fee_amt','cash_status','resp_code','resp_desc','hf_order_id','hf_order_date','created_at','updated_at'];
    /**
     * 新增
     *
     * @param array $data
     ...
     * @param $filter 更新的条件
     */
    public function lists($filter, $cols = '*', $page = 1, $pageSize = -1, $orderBy = ['created_at' => 'desc'])
    {
        $result['total_count'] = $this->count($filter);
        if ($result['total_count'] > 0) {
            $conn = app('registry')->getConnection('default');
            $colArr = explode(',', $cols);
            array_walk($colArr, function (&$value) {
                $value = 'a.' . $value;
            });
            $cols = implode(',', $colArr) . ',b.name as distributor_name';
            ...
            if ($filter['distributor_id'] ?? 0) {
                ...
                $filter['a.distributor_id'] = $distributorId;
            }
            $qb = $this->_filter($filter, $qb);
            if ($orderBy) {
                foreach ($orderBy as $filed => $val) {
    ...
    public function count($filter)
    {
        ...
        $qb->select('count(hfpay_cash_record_id)')
             ->from($this->table);
        if ($filter) {
