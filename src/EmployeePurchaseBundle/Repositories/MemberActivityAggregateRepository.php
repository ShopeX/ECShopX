...
class MemberActivityAggregateRepository extends EntityRepository
{
    public $table = 'employee_purchase_member_activity_aggregate';
    public $cols = ['id', 'company_id', 'enterprise_id', 'user_id', 'activity_id', 'aggregate_fee', 'created', 'updated'];
    ...
    {
        $entity = new MemberActivityAggregate();
    ...
    public function sum($filter)
    {
        $conn = app('registry')->getConnection('default');
        $qb = $conn->createQueryBuilder();
        $qb->select('sum(aggregate_fee)')
            ->from($this->table);
        if ($filter) {
            $this->_filter($filter, $qb);
        }
