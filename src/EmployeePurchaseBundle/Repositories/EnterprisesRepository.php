...
class EnterprisesRepository extends EntityRepository
{
    public $table = 'employee_purchase_enterprises';
    public $cols = ['id', 'company_id', 'distributor_id', 'operator_id', 'qr_code_bg_image', 'is_employee_check_enabled', 'name', 'enterprise_sn', 'logo', 'auth_type', 'disabled', 'sort', 'created', 'updated'];
     ...
     * @param $filter 更新的条件
     */
    public function getListsRelSuffix($filter, $cols = '*', $page = 1, $pageSize = -1, $orderBy = array())
    {
        $conn = app('registry')->getConnection('default');
        $qb = $conn->createQueryBuilder()->select($cols)->from($this->table, 'e');
        $qb = $qb->leftjoin('e', 'employee_purchase_enterprise_email_box', 'eb', 'e.id = eb.enterprise_id');
        $qb = $this->_filter($filter, $qb);
        if ($orderBy) {
            foreach ($orderBy as $filed => $val) {
                $qb->addOrderBy($filed, $val);
