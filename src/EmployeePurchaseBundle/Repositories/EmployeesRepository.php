...
{
    public $table = 'employee_purchase_employees';
    public $cols = ['id', 'company_id', 'distributor_id', 'operator_id', 'name', 'mobile', 'account', 'email', 'enterprise_id', 'auth_code', 'user_id', 'member_mobile', 'created', 'updated', 'disabled'];
    ...
    public function getEmployeeListWithRel($filter, $page = 1, $pageSize = -1, $orderBy = array())
    {
        ...
        $qb = $qb->select('count(*)')
            ...
            ->leftJoin('ep', 'employee_purchase_enterprises', 'et', 'ep.enterprise_id = et.id');

        if (isset($filter['enterprise_sn']) && $filter['enterprise_sn']) {
            $qb = $qb->andWhere($qb->expr()->eq('et.enterprise_sn', $qb->expr()->literal($filter['enterprise_sn'])));
            unset($filter['enterprise_sn']);
        }

        if (isset($filter['auth_type']) && $filter['auth_type']) {
            $qb = $qb->andWhere($qb->expr()->eq('et.auth_type', $qb->expr()->literal($filter['auth_type'])));
            unset($filter['auth_type']);
        }

        $qb = $this->_filter($filter, $qb, 'ep');

        $result['total_count'] = (int)$qb->execute()->fetchColumn();
        if ($result['total_count'] > 0) {
            if ($orderBy) {
                foreach ($orderBy as $filed => $val) {
                    $qb->addOrderBy('ep.'.$filed, $val);
                }
            }
            if ($pageSize > 0) {
                $qb->setFirstResult(($page - 1) * $pageSize)
            ...
            $lists = $qb->select('ep.*,et.enterprise_sn,et.name as enterprise_name,et.auth_type')->execute()->fetchAll();
            foreach ($lists as $key => $row) {
                $lists[$key]['name'] = fixeddecrypt($row['name']);
                if ($row['mobile']) {
                    $lists[$key]['mobile'] = fixeddecrypt($row['mobile']);
                }
            }
        }
        $result['list'] = $lists ?? [];
