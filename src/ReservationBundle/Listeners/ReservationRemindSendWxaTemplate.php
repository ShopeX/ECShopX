...
class ReservationRemindSendWxaTemplate implements ShouldQueue
{
    ...
    public function handle(ReservationFinishEvent $event)
    {
        try {
            ...
            $delay = $params['setting_data']['smsDelay'] ?: 1;
            //到店时间
            $endTime = $toShopTime - ($delay * 3600);
            ...
            $recordData = $reservationRecord->get(['record_id' => $recordId]);
            if ($recordData->getStatus() != 'success') {
                app('log')->debug('预约到店发送消息通知出错：预约的状态不是预约成功状态');
                return true;
            }
            if (!($params['wxapp_appid'] ?? '')) {
                ...
                if (!$unionid) {
                    app('log')->debug('预约到店发送消息通知出错：用户信息有误');
                    return true;
                }
            ...
            $wxaTemplateMsgData = [
                ...
                'remarks' => '已临近预约到店时间，请尽快到店',
            ];
            $sendData['scenes_name'] = 'reservationRemind';
            ...
            app('log')->debug('预约到店发送消息通知出错：' . $e->getMessage());
        }
    }
}
