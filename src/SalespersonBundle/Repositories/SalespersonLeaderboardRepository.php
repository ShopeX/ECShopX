...
class SalespersonLeaderboardRepository extends EntityRepository
{
    public $table = "salesperson_leaderboard";
    public $cols = ['id','company_id','date','salesperson_id','distributor_id','sales','number'];
    /**
     * 新增
     *
     * @param array $data
    ...
    public function add($fieldNumber, $filter, $isIncrement = true)
    {
        ...
        if (!$result) {
            ...
            $data['sales'] = $fieldNumber['sales'] ?? 0;
            $data['number'] = $fieldNumber['number'] ?? 0;
            $this->create($data);
        } else {
