...
class SalesPromotionsRepository extends EntityRepository
{
    public $table = "companys_sales_promotions";
    public $cols = ['sales_promotion_id','salesperson_id','unique_key','promotion_items','company_id','distributor_id'];
    /**
     * 新增
     *
     * @param array $data
    ...
    private function getColumnNamesData($entity, $cols = [], $ignore = [])
    {
        ...
        $values['promotion_items'] = json_decode($values['promotion_items'], true);
        return $values;
    }
    ...
    public function count($filter)
    {
        ...
        $qb->select('count(sales_promotion_id)')
             ->from($this->table);
        if ($filter) {
