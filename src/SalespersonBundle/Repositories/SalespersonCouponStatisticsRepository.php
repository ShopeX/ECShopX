...
class SalespersonCouponStatisticsRepository extends EntityRepository
{
    public $table = "salesperson_coupon_statistics";
    public $cols = ['id','company_id','distributor_id','salesperson_id','coupon_id','date','send_num','pay_num','receive_num','reg_num'];
    /**
     * 新增
     *
     * @param array $data
    ...
    {
        $entity = new SalespersonCouponStatistics();
     ...
     * 累加参数
     */
    public function add($fieldNumber, $filter, $isIncrement = true)
    {
        $conn = app('registry')->getConnection('default');
        $qb = $conn->createQueryBuilder();
        ...
        if (!$result) {
            ...
            $data['distributor_id'] = $filter['distributor_id'];
            $data['salesperson_id'] = $filter['salesperson_id'];
            $data['coupon_id'] = $filter['coupon_id'];
            $data['date'] = $filter['date'];
            $data['send_num'] = $fieldNumber['send_num'] ?? 0;
            $data['pay_num'] = $fieldNumber['pay_num'] ?? 0;
            $data['receive_num'] = $fieldNumber['receive_num'] ?? 0;
            $data['reg_num'] = $fieldNumber['reg_num'] ?? 0;
            $this->create($data);
        } else {
    ...
    public function getCountGroupByCouponsId($filter)
    {
        $conn = app('registry')->getConnection('default');
        $qb = $conn->createQueryBuilder();
        $qb->select('coupon_id, sum(send_num) send_num, count(pay_num) pay_num, count(receive_num) receive_num, count(reg_num) reg_num')
            ->from($this->table)
            ->groupBy('coupon_id');
        if ($filter) {
            $this->_filter($filter, $qb);
