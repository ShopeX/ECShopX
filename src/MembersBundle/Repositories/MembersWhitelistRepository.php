...
class MembersWhitelistRepository extends EntityRepository
{
    public $table = "members_whitelist";
    public $cols = ['whitelist_id','company_id','mobile','name','created','updated'];
    /**
     * 新增
     *
     * @param array $data
    ...
    public function lists($filter, $cols = '*', $page = 1, $pageSize = -1, $orderBy = array())
    {
        ...
        if ($result['list']) {
            foreach ($result['list'] as $key => $value) {
                $result['list'][$key]['mobile'] = fixeddecrypt($value['mobile']);
                $result['list'][$key]['name'] = fixeddecrypt($value['name']);
            }
        }
        return $result;
    }
