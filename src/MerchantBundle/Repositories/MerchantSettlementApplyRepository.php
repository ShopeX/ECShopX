...
class MerchantSettlementApplyRepository extends EntityRepository
{
    public $table = "merchant_settlement_apply";
    public $cols = ['id','company_id','mobile','is_agree_agreement','merchant_type_id','settled_type','merchant_name','social_credit_code_id','province','city','area','regions_id','address','legal_name','legal_cert_id','legal_mobile','bank_acct_type','card_id_mask','bank_name','bank_mobile','license_url','legal_certid_front_url','legal_cert_id_back_url','bank_card_front_url','audit_status','audit_memo','source','audit_goods','disabled','created','updated'];
    /**
     * 新增
     *
     * @param array $data
    ...
    {
        $entity = new MerchantSettlementApply();
    ...
    public function updateOneBy(array $filter, array $data)
    {
        ...
        if (!$entity) {
            throw new ResourceException(trans("MerchantBundle.no_update_data_found"));
        }
    ...
    public function lists($filter, $cols = '*', $page = 1, $pageSize = -1, $orderBy = array())
    {
        ...
        foreach ($result['list'] as $key => $list) {
            ...
            $list['legal_cert_id'] = fixeddecrypt((string) $list['legal_cert_id']);
            $list['legal_mobile'] = fixeddecrypt((string) $list['legal_mobile']);
            $list['card_id_mask'] = fixeddecrypt((string) $list['card_id_mask']);
            $list['bank_mobile'] = fixeddecrypt((string) $list['bank_mobile']);
    ...
    private function formatData($data)
    {
        ...
        if (isset($data['is_agree_agreement']) && is_numeric($data['is_agree_agreement'])) {
            $data['is_agree_agreement'] = $data['is_agree_agreement'] == '1';
        }
        return $data;
    }
