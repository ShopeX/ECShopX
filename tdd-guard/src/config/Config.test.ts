...
describe('Config', () => {
  ...
  describe('useSystemCursor', () => {
    test('can be set via options', () => {
      const config = new Config({ useSystemCursor: true })

      expect(config.useSystemCursor).toBe(true)
    })

    test('options take precedence over env var', () => {
      process.env.USE_SYSTEM_CURSOR = 'false'
    ...
    test('falls back to env var when not in options', () => {
      process.env.USE_SYSTEM_CURSOR = 'true'
    ...
    test('defaults to false when neither options nor env var are set', () => {
      delete process.env.USE_SYSTEM_CURSOR

      const config = new Config()

      expect(config.useSystemCursor).toBe(false)
    })
  })
  ...
  describe('validationClient', () => {
    test.each<ClientType>(['sdk', 'cli', 'api', 'cursor-cli'])(
    ...
    describe('options.validationClient precedence', () => {
      test.each([
        ...
        ({ env, envValue, optionValue }) => {
          ...
          const config = new Config({
            validationClient: optionValue as
              | 'api'
              | 'cli'
              | 'sdk'
              | 'cursor-cli',
    ...
    test('uses cursor-cli from options.modelType=cursor_cli when validationClient not set', () => {
      delete process.env.VALIDATION_CLIENT
      delete process.env.MODEL_TYPE

      const config = new Config({ modelType: 'cursor_cli' })

      expect(config.validationClient).toBe('cursor-cli')
    })
